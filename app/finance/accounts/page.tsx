...
import { resolveDisplayedAccountBalance } from "@/lib/finance/recalculateAccountBalances";
...
export default async function FinanceAccountsPage({
  ...
  const [companies, categories, accounts, transactions] = await Promise.all([
  ...
  for (const account of accounts) {
    const key = `${account.companyName}|||${account.name}`;

    accountMap.set(key, {
      companyName: account.companyName,
      bankAccount: account.name,
      openingBalance: toAmount(account.openingBalance),
      balance: resolveDisplayedAccountBalance(account),
      inflow: 0,
      outflow: 0,
      operationsCount: 0,
      ignoredCount: 0,
  ...
  for (const operation of transactions) {
    // PLAN must not affect current balance ownership or confuse activity effects.
    if (operation.transactionStatus === "PLAN") continue;
    ...
    // Activity attaches only to configured FinanceAccount registry rows.
    if (!current) continue;

    const effect = getFinanceTransactionAccountEffect(
      operation,
      ...
      <div className="mx-auto max-w-[1600px] space-y-6">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
          <div>
            ...
            <p className="mt-3 text-slate-500">
              Остатки берутся из реестра счетов (FinanceAccount). Активность
              (поступления / выбытия) считается по операциям через единую модель
              ДДС с учётом роли статьи profitTreatment и переводов.
            </p>
          </div>
        ...
        <section className="grid gap-4 sm:grid-cols-2 xl:grid-cols-6">
          <div className="rounded-2xl bg-white p-6 shadow-sm">
            <div className="text-sm text-slate-500">Счетов в реестре</div>
            ...
            <div className="mt-5 space-y-4">
              ...
              {accountRows.length === 0 && (
                <div className="rounded-xl border border-slate-200 p-4 text-slate-500">
                  Активных счетов в реестре пока нет.
                </div>
              )}
            </div>
          </div>
        ...
        <section className="rounded-2xl bg-white p-6 shadow-sm">
          <div>
            ...
            <p className="mt-2 text-slate-500">
              Текущий остаток — из реестра FinanceAccount. Поступления и выбытия
              — по фактическим операциям (без PLAN), с учётом переводов на уровне
              счёта.
            </p>
          </div>
            ...
            <table className="w-full min-w-[1150px] text-sm">
              <thead className="bg-slate-100 text-left text-slate-700">
                <tr>
                  ...
                  <th className="p-3 text-right">Текущий остаток</th>
