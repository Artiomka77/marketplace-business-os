...
export default async function FinanceCalendarPage({
  ...
  const loanPaymentsRaw = await prisma.loanPayment.findMany({
    where: {
      paymentDate: {
        gte: monthStart,
        lte: monthEnd,
  ...
  const allFutureLoanPaymentsRaw = await prisma.loanPayment.findMany({
    where: {
      paymentDate: {
        gte: today,
        lte: thirtyDaysEnd,
