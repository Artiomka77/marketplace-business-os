...
import QuickAddFinanceHelper from "./QuickAddFinanceHelper";
import {
...
export default async function FinanceOperationsPage({
  ...
  const metrics = calculateFinanceMetricsForRows({
    transactions: metricRows.filter(
      (row) => String(row.transactionStatus ?? "FACT").toUpperCase() === "FACT",
    ),
    categories,
  });
