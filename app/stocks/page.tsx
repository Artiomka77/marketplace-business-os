...
import { loadStockPlanningAbcPack } from "@/lib/consumers/waveDE";
import { WaveDEConsumerUnavailableError } from "@/lib/consumers/waveDE";
...
function ProductionPlanningBlock({
  ...
  const productionPlanRows = allProductionPlanRows;
  ...
  const selectedProductionPlanQty = productionPlanSummaryQty(selectedProductionPlanRows);
  ...
  function makeProductionExportHref(bufferDays: number) {
    ...
    if (hasCustomProductionSelection && selectedProductionKeys.size > 0) {
                ...
                {productionPlanRows.length > 0 ? (
                  <>
                    ...
                    <div className="flex flex-col gap-2 border-t border-amber-100 bg-white/50 px-4 py-3 text-xs font-bold text-slate-500 sm:flex-row sm:items-center sm:justify-between">
                      <div>
                        Выбрано к пошиву: {formatNumber(selectedProductionPlanRows.length)} из {formatNumber(allProductionPlanRows.length)} позиций. По умолчанию все найденные позиции отмечены галками; лишние можно снять и нажать “Применить выбор”.
                      </div>
                      <button
                        form="production-plan-form"
...
export default async function StocksPage({
  ...
  let stockPlanningMeta: Awaited<ReturnType<typeof loadStockPlanningAbcPack>>["meta"] | null = null;
  let stockAbcUnavailable: { reason: string; message: string } | null = null;
  let wbAbcMap = new Map<string, StockAbcInfo>();
  let ozonAbcMap = new Map<string, StockAbcInfo>();
  let ozonSalesEntries: Array<{ companyName: string; vendorCode: string; netSalesQty: number; revenue: number }> = [];
  try {
    const { pack: stockAbcSnapshots, meta } = await loadStockPlanningAbcPack({
      companyScope: selectedCompanyName ?? "ALL",
      dateFrom: abcDateFrom,
      dateTo: abcDateTo,
      requireExact: true,
    });
    stockPlanningMeta = meta;
    for (const entry of stockAbcSnapshots.wbEntries) {
      registerStockAbc(wbAbcMap, {
        companyName: entry.companyName,
        article: entry.article,
        abc: { abcByRevenue: entry.abcByRevenue, abcByProfit: entry.abcByProfit },
      });
    }
    ...
    ozonSalesEntries = stockAbcSnapshots.ozonSalesEntries;
  } catch (error) {
    if (error instanceof WaveDEConsumerUnavailableError) {
      stockAbcUnavailable = { reason: error.reason, message: error.message };
      wbAbcMap = new Map();
      ozonAbcMap = new Map();
    } else {
      throw error;
  ...
  for (const row of ozonSalesEntries) {
    const vendorCode = normalizeKey(row.vendorCode);
            ...
            <div className="mt-4 space-y-3">
              {summaries.map((summary) => (
                <article
                  ...
                  className="grid gap-3 rounded-[24px] border border-slate-200 bg-white p-3 shadow-sm xl:grid-cols-[minmax(220px,1.3fr)_170px_170px_190px_150px_170px_44px] xl:items-center"
                >
                  <div>
                    <h3 className="text-lg font-black text-slate-950">
                      {summary.companyName}
                  ...
                  <Link
                    ...
                    className="flex h-11 w-11 items-center justify-center rounded-full border border-slate-200 bg-white text-lg font-black text-slate-500 transition hover:bg-slate-50"
                    aria-label={`Открыть ${summary.companyName}`}
                  >
                    →
                  </Link>
                </article>
