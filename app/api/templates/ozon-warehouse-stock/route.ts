...
export async function GET() {
  ...
  const stockSheet = workbook.addWorksheet("Остатки_склада", {
  ...
  stockSheet.addRows([
    [
      ...
      850,
      "23.06.2026",
      "Пример строки. Замените на свои данные.",
    ],
    [
      safeCompanyNames[0],
      "914803449-146",
  ...
  guideSheet.addRows([
    [
      "Компания",
      "Выберите компанию из списка. Для Ozon-планирования сейчас используем ИП Петров, но шаблон поддерживает несколько компаний.",
      "Да",
    ],
    [
      "Артикул",
      "Артикул продавца из Ozon. Именно это поле используется для выгрузки файлов поставки в Ozon.",
      "Да",
    ],
    [
      "SKU Ozon",
      "Числовой SKU Ozon. Поле желательно для сверки с Ozon, но не обязательно для первого MVP.",
      "Нет",
    ],
    ["Название товара", "Название для удобства проверки.", "Нет"],
    ["Цвет", "Цвет товара, если удобно вести учёт по цвету.", "Нет"],
    ["Размер", "Размер товара, если удобно вести учёт по размеру.", "Нет"],
    ["Штрихкод", "Баркод/штрихкод товара, если есть.", "Нет"],
    [
  ...
  return new NextResponse(Buffer.from(buffer), {
    ...
    headers: {
      ...
      "Content-Disposition":
        'attachment; filename="ozon_warehouse_stock_template.xlsx"',
