...
export async function GET(req: Request) {
  ...
  try {
    ...
    if (orderStats.ok === false) {
      logMarketplaceOrdersEvent({
        runId: createOrderSyncRunId(),
        route: "/api/cron/daily-priority-sync",
        mode: "primary",
        targetDate: dateText,
        phase: "order_stats",
        status: orderStats.retryable ? "retryable_incomplete" : "terminal_incomplete",
        retryable: orderStats.retryable,
        incompleteCount: orderStats.incompleteRequired?.length ?? 0,
        recoveryPlanned: true,
        reasonCode: "PRIMARY_INCOMPLETE_RECOVERY_EXPECTED",
        partialSuccess: orderStats.partialSuccess,
      });
    }
