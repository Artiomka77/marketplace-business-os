...
async function loadOzonDailySourceFreshness(date: string | null) {
  ...
  const [realizations, sessions, job] = await Promise.all([
    prisma.ozonRealizationSummary.findMany({
      where: {
    ...
    prisma.importSession.findMany({
      ...
      take: 80,
      select: { companyName: true, status: true, previewJson: true, fileName: true },
    }),
  ...
  return companies.map((companyName) => {
    const companySessions = sessions.filter((row) => row.companyName === companyName);
    const exactSession =
      companySessions.find((row) => {
        const preview =
          row.previewJson && typeof row.previewJson === "object"
            ? (row.previewJson as Record<string, unknown>)
            : null;
        if (!preview) return String(row.fileName ?? "").includes(date);
        ...
        return Array.isArray(persisted) && persisted.map(String).includes(date);
      }) ?? companySessions[0];

    const preview =
      exactSession?.previewJson && typeof exactSession.previewJson === "object"
        ? (exactSession.previewJson as Record<string, unknown>)
        : null;
    const diagnostics =
      preview?.diagnostics && typeof preview.diagnostics === "object"
        ? (preview.diagnostics as Record<string, unknown>)
        : null;

    return classifyOzonDailySourceFreshness({
      companyName,
      date,
      realizationRowCount: realizations.filter((row) => row.companyName === companyName).length,
      latestImportStatus: exactSession?.status ?? null,
      latestJobError: job?.errorMessage ?? null,
      latestIngestStatus: typeof preview?.ingestStatus === "string" ? preview.ingestStatus : null,
      latestFailFinality: typeof preview?.failFinality === "boolean" ? preview.failFinality : null,
      latestGrossExpenseDifference:
        typeof diagnostics?.grossExpenseDifference === "number"
          ? diagnostics.grossExpenseDifference
          : null,
      latestCoverageComplete:
        typeof preview?.coverageComplete === "boolean" ? preview.coverageComplete : null,
      previewJson: exactSession?.previewJson ?? null,
    });
  });
}

function isoDateFromDailyReport(report: { dateLabel?: string }) {
