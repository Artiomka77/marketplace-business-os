...
type InternalDailyPoint = DashboardDailyPoint & {
  ...
  taxesAmount: number;
  /** Full-precision WB tax portion before legal-entity rounding alignment. */
  wbTaxesAmount: number;
...
function addTaxForRevenue(
  ...
  point.taxesAmount +=
    revenue * (settings.usnRate / 100) +
    calculateVatTax(revenue, settings.vatRate);
}

function addTaxForWbAccountantBase(
  point: InternalDailyPoint,
  input: {
    buyerPaid: number;
    operation: "SALE" | "RETURN" | "OTHER";
    companyName: string | null | undefined;
    reportNumber: string | null | undefined;
    kindByKey: Map<string, WbFinanceTaxReportKind | "ambiguous">;
  },
  taxByCompany: Map<string, CompanyTaxSettings>,
  taxesByCompanyName: Map<string, number>
): { blocked: boolean; reason: string | null } {
  try {
    const settings = getCompanyTaxes(input.companyName, taxByCompany);
    const reportKind = resolveWbSaleTaxReportKind({
      companyName: input.companyName,
      reportNumber: input.reportNumber,
      kindByKey: input.kindByKey,
      requireResolved: input.operation !== "OTHER" && input.buyerPaid > 0,
    });
    const delta = calculateWbAccountantTaxContribution({
      buyerPaid: input.buyerPaid,
      operation: input.operation,
      reportKind,
      usnRate: settings.usnRate,
      vatRate: settings.vatRate,
    }).taxAmountDelta;
    point.taxesAmount += delta;
    point.wbTaxesAmount += delta;
    const companyKey = cleanText(input.companyName) || "__unknown__";
    taxesByCompanyName.set(
      companyKey,
      (taxesByCompanyName.get(companyKey) ?? 0) + delta
    );
...
export async function getDashboardDailyAnalytics(params: {
  ...
  const [costs, taxByCompany, ozonVendorCodeBySku] = await sequentialAll([
    ...
    async () => (getTaxByCompany()),
    async () => (getOzonVendorCodeBySku()),
  ]);

  const costByVendorCode = buildCostByVendorCode(costs);

  const [wbSales, wbFinanceKinds, wbAdsRaw, ozonFinance, ozonAdsRaw, financeTransactions, financeCategories] =
    await sequentialAll([
    ...
    async () => (prisma.wbFinance.findMany({
        where: {
          ...(companyName ? { companyName } : {}),
          OR: [
            { dateFrom: { gte: from, lt: toExclusive } },
            { dateTo: { gte: from, lt: toExclusive } },
  ...
  const wbTaxReportKindByKey = buildWbFinanceTaxReportKindByKeyForRelevantSales(
    wbFinanceKinds,
    wbSales
  );
  const wbTaxesByCompanyName = new Map<string, number>();
  let wbTaxBlockedReason: string | null = null;
  ...
  for (const row of wbSales) {
    ...
    const payment = normalizeText(row.paymentReason);
    const doc = normalizeText(row.documentType);
    const isStorno = payment === "сторно возвратов" || doc === "сторно возвратов";
    const isReturn =
      !isStorno &&
      (isWbReturnOperation(row.paymentReason) ||
        isWbReturnOperation(row.documentType));
    const isSale =
      !isReturn &&
      (isWbSaleOperation(row.paymentReason) ||
        isWbSaleOperation(row.documentType) ||
        isStorno);
    const operation = isSale ? "SALE" : isReturn ? "RETURN" : "OTHER";
    ...
    const taxResult = addTaxForWbAccountantBase(
      point,
      {
        buyerPaid: Math.abs(realizedAmount),
        operation,
        companyName: row.companyName,
        reportNumber: row.reportNumber,
        kindByKey: wbTaxReportKindByKey,
      },
      taxByCompany,
      wbTaxesByCompanyName
    );
    if (taxResult.blocked) {
      wbTaxBlockedReason = taxResult.reason;
    }
  }

  if (wbTaxBlockedReason) {
    // Fail-closed for unresolved WB report kinds: strip WB tax contributions so
    // the dashboard remains renderable without inventing ordinary tax.
    for (const point of pointsByDate.values()) {
      if (point.wbTaxesAmount === 0) continue;
      point.taxesAmount -= point.wbTaxesAmount;
      point.wbTaxesAmount = 0;
    }
    wbTaxesByCompanyName.clear();
  } else {
  // Align period WB tax to legal-entity rounding: apply delta once on the last
  // day that carries WB tax so sum(day.wbTaxes) == sum(round(entity)).
  const roundedWbTax = aggregateRoundedWbTaxLiabilities([
    ...wbTaxesByCompanyName.values(),
  ]);
  const rawWbTax = Array.from(pointsByDate.values()).reduce(
    (sum, point) => sum + point.wbTaxesAmount,
    0
  );
  const wbRoundingDelta = roundedWbTax - rawWbTax;
  if (Math.abs(wbRoundingDelta) > 1e-9) {
    const points = Array.from(pointsByDate.values()).filter(
      (point) => Math.abs(point.wbTaxesAmount) > 1e-12
    );
    const target = points.at(-1);
    if (target) {
      target.wbTaxesAmount += wbRoundingDelta;
      target.taxesAmount += wbRoundingDelta;
    }
  }
  }
