...
export async function getDataReadinessSummary(
  ...
  const [
    ...
    async () => (prisma.marketplaceDailyOrderStat.count({
      where: buildOrderWhere(safeCompanyNames, startDate, dateToExclusive),
    })),
    async () => (prisma.wbFinance.count({
      where: {
        ...companyWhere,
    ...
    async () => (prisma.wbSale.count({
      where: {
        ...companyWhere,
        saleDate: dateRange,
      },
    })),
    async () => (prisma.wbAds.count({
      where: {
        ...companyWhere,
        dateFrom: { lte: reportEndInclusive },
        dateTo: { gte: startDate },
    ...
    async () => (prisma.ozonFinance.count({
      where: {
        ...companyWhere,
    ...
    async () => (prisma.ozonAds.count({
      where: {
        ...companyWhere,
        reportDate: dateRange,
      },
    })),
    async () => (prisma.ozonFinance.groupBy({
      by: ["companyName"],
      where: {
        ...companyWhere,
        accrualDate: dateRange,
  ...
  for (const activityCompanyName of ozonActivityCompanyNames) {
    const [realizationRows, discountRows] = await sequentialAll([
    async () => (prisma.$queryRaw<PeriodCoverageRow[]>`
        SELECT
          "companyName",
