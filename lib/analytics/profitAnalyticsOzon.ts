...
function recalculateOzonRowsFromEconomicModel(
  ...
  for (const row of rows) {
    ...
    const usnTaxBundle = calculateMarketplaceTotalTax({
      ordinarySalesVatInclusive: Math.max(0, row.revenue),
      separateVatTaxableAmount: Math.max(0, allocatedPartnerPrograms),
      usnRate,
      vatRate,
    });

    row.taxesAmount = usnTaxBundle;
    row.marginProfit =
...
function applyOzonEconomicModel(
  ...
  result.totals.taxCalculationMode = realizationCoverageComplete
    ? "FINAL_TAXABLE_REVENUE"
    : "ESTIMATED_ECONOMIC_TURNOVER";
  // Ordinary tax base is realized sales only — never re-add discount points.
  // Partner-program VAT-taxable amount is applied separately when realization is FINAL.
  result.totals.taxCalculationBase = result.totals.revenue;
  result.totals.taxesEstimated = !realizationCoverageComplete;
  result.totals.netProfitStatus =
    realizationCoverageComplete && !result.totals.costCoverageIncomplete
    ? "FINAL"
    : "PRELIMINARY";

  const separateVatTaxableAmount = realizationCoverageComplete
    ? partnerProgramsAmount
    : 0;

  result.totals.taxesAmount = calculateMarketplaceTotalTax({
    ordinarySalesVatInclusive: result.totals.taxCalculationBase,
...
function mergeOzonTotals(
  ...
  for (const totals of totalsList) {
    ...
    result.costCoverageIncomplete =
      Boolean(result.costCoverageIncomplete) ||
      Boolean(totals.costCoverageIncomplete);
  ...
  result.netProfitStatus =
    result.taxRevenueCoverageComplete && !result.costCoverageIncomplete
    ? "FINAL"
    : "PRELIMINARY";
...
function mergeOzonRows(rowsList: OzonProfitAnalyticsRow[][]) {
  ...
  for (const rows of rowsList) {
    for (const row of rows) {
      ...
      current.costResolved =
        Boolean(current.costResolved) && Boolean(row.costResolved);
      current.costCoverageIncomplete =
        Boolean(current.costCoverageIncomplete) ||
        Boolean(row.costCoverageIncomplete);
...
function calculateRowsAndTotals({
  ...
  for (const financeRow of marketplaceFinanceRows) {
    ...
    // Cost of goods belongs only to real product sales and returns.
    // Service rows may carry quantity and positive reversals, so quantity or
    // totalAmount must never be used as sale/return classifiers.
    if (salesAmount > 0) {
    ...
    current.taxesAmount = calculateMarketplaceTotalTax({
      ordinarySalesVatInclusive: Math.max(0, current.revenue),
    ...
    for (const companyName of OZON_COMPANY_NAMES) {
      companyResults.push(
        await getProfitAnalyticsOzon({
          ...params,
          companyName,
          // Mixed-company analytics must always use each company's saved tax
          // settings. Legacy page query parameters must not override them.
          usnRate: undefined,
          vatRate: undefined,
        })
      );
