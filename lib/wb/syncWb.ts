...
async function fetchWbFinanceReports(
  ...
  if (!result.ok) {
    throw new Error(
      `WB Finance API: ${result.status} ${formatWbFinanceBody(result.body)}`.trim(),
    );
  }
  ...
  return {
    dateFrom,
    dateTo,
    rows: result.body as WbFinanceReport[],
  };
}
...
async function fetchWbSalesDetailedRows(token: string, reportId: string) {
  ...
  while (true) {
    ...
    if (!result.ok) {
      throw new Error(
        `WB Sales API report ${reportId}: ${result.status} ${formatWbFinanceBody(result.body)}`.trim(),
      );
    }
...
export async function syncWbSalesByReportNumber(
  ...
  if (salesNormalizeResult.savedRows > 0) {
    const { safeInvalidateWaveBProfitReadModel } = await import(
      "@/lib/profitReadModel/invalidation"
    );
    await safeInvalidateWaveBProfitReadModel({
