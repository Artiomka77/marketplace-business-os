...
async function fetchWbDailyFinanceReports(
  ...
  if (!result.ok) {
    throw new Error(
      `WB Daily Finance API: ${result.status} ${formatWbFinanceBody(result.body)}`.trim(),
    );
  }
  ...
  return { dateFrom, dateTo, period, rows: result.body as WbFinanceReport[] };
}
...
async function fetchWbDetailedReportRows(token: string, reportId: string) {
  ...
  for (let page = 0; page < maxPages; page += 1) {
    const result = await wbFinanceRequest({
      url: WB_FINANCE_REPORT_DETAILS_BY_ID_URL(reportId),
      token,
      body: {
        limit,
        rrdId,
    ...
    if (!result.ok) {
      throw new Error(
        `WB Daily Detail report ${reportId}: ${result.status} ${formatWbFinanceBody(result.body)}`.trim(),
      );
    }
...
export async function syncWbDailyFinancialReports(
  ...
  try {
    ...
    if (financeNormalizeResult.savedRows > 0 || detailedRows > 0) {
