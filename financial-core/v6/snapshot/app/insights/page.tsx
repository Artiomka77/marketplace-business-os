...
export default async function InsightsPage({
    ...
    await Promise.all([
      Promise.all(
        companyNames.map(async (companyName) => {
          ...
          applyOzonCanonicalIngestFinality(ozon.totals, ozonFinality);
          const wbUnavailable = isProfitAnalyticsUnavailable(wb);
          const combined = wbUnavailable
            ? {
                insights: "PRELIMINARY" as const,
              }
            : resolveCanonicalPeriodFinality({
            wbSelected: true,
            ozonSelected: true,
            wb: resolveWbPeriodFinality({
              dataMode: wb.totals.dataMode,
              sourceOwnershipMode: wb.totals.sourceOwnershipMode,
              sourceOwnershipFinal: wb.totals.sourceOwnershipFinal,
              sourceOwnershipReasons: wb.totals.sourceOwnershipReasons,
            }),
            ozon: ozonFinality,
          });
  ...
  return (
    <main
      ...
      data-insights-combined-complete={wbUnavailable ? "no" : "yes"}
    >
