...
import { loadStockPlanningAbcPack, WaveDEConsumerUnavailableError } from "@/lib/consumers/waveDE";
import {
  assertWbFinancialDataFinal,
  isWbPreliminaryFinancialDataError,
  requiresFinalWbFinancialDataForMarketplace,
  WB_PRELIMINARY_FINANCIAL_DATA,
...
async function buildSupplyPlanRows(url: URL) {
  ...
  const requestedExportMarketplace = getExportMarketplace(
    getQueryValue(url, "exportMarketplace")
  );
  const requestedSupplyMarketplace = getMarketplaceFilter(
    getQueryValue(url, "supplyMarketplace")
  );

  let wbAbcMap = new Map<string, StockAbcInfo>();
  let ozonAbcMap = new Map<string, StockAbcInfo>();
  let waveDEMeta: Awaited<ReturnType<typeof loadStockPlanningAbcPack>>["meta"] | null = null;
  let ozonSalesEntries: Array<{ companyName: string; vendorCode: string; netSalesQty: number; revenue: number }> = [];
  try {
    const { pack, meta } = await loadStockPlanningAbcPack({
    ...
    waveDEMeta = meta;
    ozonSalesEntries = pack.ozonSalesEntries ?? [];
    for (const entry of pack.wbEntries) {
      registerStockAbc(wbAbcMap, { companyName: entry.companyName, article: entry.article, abc: { abcByRevenue: entry.abcByRevenue, abcByProfit: entry.abcByProfit } });
    }
    for (const entry of pack.ozonEntries) {
      registerStockAbc(ozonAbcMap, { companyName: entry.companyName, article: entry.article, abc: { abcByRevenue: entry.abcByRevenue, abcByProfit: entry.abcByProfit } });
    }
  } catch (error) {
    if (!(error instanceof WaveDEConsumerUnavailableError)) throw error;
    throw error;
  }

  if (
    requiresFinalWbFinancialDataForMarketplace({
      exportMarketplace: requestedExportMarketplace,
      supplyMarketplace: requestedSupplyMarketplace,
    })
  ) {
    assertWbFinancialDataFinal(
      waveDEMeta?.dataMode === "FINAL"
        ? { dataMode: "FINAL" as const, sourceOwnershipFinal: true, sourceOwnershipReasons: [] as string[] }
        : null,
      "SUPPLY_PLAN_EXPORT"
    );
  }
  ...
  try {
    ...
    if (error instanceof WaveDEConsumerUnavailableError) {
      return NextResponse.json(
        { ok: false, unavailable: true, reason: error.reason, message: error.message, source: "READ_MODEL" },
        { status: 409, headers: { "x-wave-de-source": "READ_MODEL", "x-wave-de-heavy-fc": "0" } },
      );
    }
    if (isWbPreliminaryFinancialDataError(error)) {
      return NextResponse.json(
        {
          success: false,
          code: WB_PRELIMINARY_FINANCIAL_DATA,
