...
async function getWbMetrics(companyName: string, range: DateRange) {
  ...
  // Единый источник истины: если каноническая WB-аналитика доступна,
  // налоговая выручка должна совпадать с /profit-wb. Локальный расчёт
  // по WbSale используется только как fallback при отсутствии аналитики.
  ...
  // Главный показатель масштаба = экономический оборот.
  // Главный ДРР = реклама / экономический оборот.
  // Налоговая выручка и ДРР от налоговой выручки показываются отдельно.
...
function getProfitConclusion(report: DailyReport) {
  ...
  if (report.totals.netProfitImpact < 0) {
    return `${label} под давлением: ${formatMoney(
      report.totals.netProfitImpact
...
export function formatDailyReportForTelegram(report: DailyReport) {
  ...
  const dataReadinessText = preliminary
    ? estimatedOzonTax
      ? "⚠️ Предварительный расчёт: прибыль до налогов Ozon рассчитана от экономического оборота; налоговый резерв и чистая прибыль уточнятся после отчёта начислений."
      : `⚠️ ${report.dataReadiness?.shortText ?? "Данные периода неполные"}: финансовый результат предварительный.`
    : "";
  ...
  const lines: string[] = [
    ...
    estimatedOzonTax
      ? `Налоговая выручка: ${formatMoney(
          report.totals.taxableRevenue
        )} по закрытым источникам + Ozon ожидается`
      : `Налоговая выручка: ${formatMoney(report.totals.taxableRevenue)}${inlinePercentChange(
          comparison?.taxableRevenuePercent ?? null
        )}`,
