...
if (!fs.existsSync(manifestPath)) {
  console.error("FINANCIAL CORE LOCK ERROR: financial-core/v3/manifest.json is missing.");
  process.exit(1);
}
...
if (failures.length > 0) {
  console.error("");
  console.error("FINANCIAL CORE V3: LOCK VIOLATION");
  console.error("Финансовое ядро V3 или его подключение изменено.");
  console.error("");
...
console.log(
  `FINANCIAL CORE V3: VERIFIED (${manifest.protectedFiles.length} protected files, ${manifest.consumerContracts.length} consumers)`
);
